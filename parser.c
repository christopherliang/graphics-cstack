...
void parse_file ( char * filename, 
  ...
  char line[256];
  struct matrix *tmp;
  struct stack * myStack = new_stack();
  double angle;
  color g;
  ...
  while ( fgets(line, 255, f) != NULL ) {
    ...
    if ( strncmp(line, "color", strlen(line)) == 0 ) {
      ...
      matrix_mult(tmp, myStack->data[myStack->top]);
      draw_lines(myStack->data[myStack->top],s,g);
      ...
      matrix_mult(myStack->data[myStack->top],tmp);
      ...
      matrix_mult(myStack->data[myStack->top],tmp);
      draw_polygons(myStack->data[myStack->top],s,g);
      //printf( "%lf %lf %lf\n", x, y, z);
    }
